class DrawCurve{
	...
	public:
		DrawCurve(){
			...
			TFile *fCurve3 = new TFile("/media/Projects/RIKEN_Cluster_2018/lipj/exp_201805/anaroot/users/analysis/plot/esBe10Excitation/protonCurveBe10.root","READ");
			gs = (TGraph *)gDirectory->Get("gs");
			fex = (TGraph *)gDirectory->Get("fex");
			fCurve3->Close();
...
class CheckEx{
	...
	public:
		...
		void setGate(){
			if(side !=-1&&barId != -1) gate = Form("isEspriLR==%d&&naiBarId==%d",side,barId);
			if(side ==-1&&barId == -1) gate ="1";
			if(side ==0&&barId == -1) gate ="isEspriLR==0";
			if(side ==1&&barId == -1) gate ="isEspriLR==1";
		}
		void drawTot(){
...
void drawBar(int side,int barId){
	...
	int markerStyle = 1;
	if(side!=-1&&barId!=-1) markerStyle = 1;
...
void checkCalibTOF(){
	...
	drawBar(0,0); // draw bar side + barId
