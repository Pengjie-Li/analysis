class ReadFile{
	...
	public:
		...
		void fillTDC(DCRaw *RawData,vector<int> *layer_hits,vector<double> *layerid,vector<double> *wireid, vector<double> *TDC){
			for (int i = 0; i < (*layer_hits).size(); ++i) {
				if((*layer_hits)[i]==2){
					...
					while(1){
						if((*layerid)[j] == i){
							fillId++;
							wireId[fillId] = (*wireid)[j];
							fillTdc[fillId] = (*TDC)[j];
						}
						if(fillId ==1) break;
						j++;
					}
					if(abs(wireId[0]-wireId[1])==1){
						RawData->fillTDC(i,fillTdc[0],fillTdc[1]);
						//cout<<"FILL!"<<endl;
						//cout<<wireId[0]<<":"<<wireId[1]<<":"<<fillTdc[0]<<":"<<fillTdc[1]<<endl;
					}
		...
		void fillBdc1(DCRaw *bdc1RawData){
			fillTDC(bdc1RawData,BDC1_layer_hits,BDC1_layerid,BDC1_wireid,BDC1_TDC);
		}
	
		void fillBdc2(DCRaw *bdc2RawData){
			fillTDC(bdc2RawData,BDC2_layer_hits,BDC2_layerid,BDC2_wireid,BDC2_TDC);
		}
		void fillFdc0(DCRaw *fdc0RawData){
			fillTDC(fdc0RawData,FDC0_layer_hits,FDC0_layerid,FDC0_wireid,FDC0_TDC);
		}
