...
class ConvertDC{
	...
	public :
		...
		void setFDCBranch(){
			...
			tree_FDC->Branch("FDC0_Track_TDC",&FDC0_Track_TDC);
		...
		void launchAnalysisLoop(){
			     ...
			     )
			{
				...
				FDC0_Track_TDC.clear();
				FDC0_Track_TDC.resize(8,-9999);
		...
		void printFDC0Track(){
			...
			cout<<"Layer = "<<i<<" Wire = "<<setw(4)<<FDC0_Track_wire[i]<<" TDC = "<<setw(4)<<FDC0_Track_TDC[i]<<" Drift length="<<setw(9)<<FDC0_Track_dl[i]<<" xypos="<<setw(9)<<FDC0_Track_XY[i]<<" zpos="<<setw(9)<<FDC0_Track_Z[i]<<" Residue dxy = "<<setw(9)<<FDC0_Track_dXY[i]<<endl;
