	...
	public:
		...
		void loadChain(int runStart,int runStop){
			for (int i = runStart; i <= runStop; ++i) {
				...
				inputTree->Add(Form("inputRootfiles/run0%d_analysed.root",i));
			}
			//tree->Print();
			setReadBranch();
		...
		bool goodFDC0Gate(){
			...
			//Beam hit on frame cauing reaction
			if(isInsideTargetArea()&&oneLayerOneHit(FDC0_layer_hits)) return true;
			return false;
		}
		bool goodFDC2Gate(){
			//if(layerHitNoMoreThan2(FDC0_layer_hits)) cout<<"FDC 0 true"<<endl;
			//if(layerHitNoMoreThan2(FDC0_layer_hits)) return true;
			//if(oneLayerOneHit(FDC2_layer_hits)) return true;
			return false;
		}
		...
		vector<double > getBDC1LayerId(){
			return (*BDC1_layerid);
		}
		vector<double > getBDC1WireId(){
			return (*BDC1_wireid);
		}
		vector<double > getBDC1TDC(){
			return (*BDC1_TDC);
		...
		vector<double > getBDC2LayerId(){
			return (*BDC2_layerid);
		}
		vector<double > getBDC2WireId(){
			return (*BDC2_wireid);
		}
		vector<double > getBDC2TDC(){
			return (*BDC2_TDC);
		...
		vector<double > getFDC0LayerId(){
			return (*FDC0_layerid);
		}
		vector<double > getFDC0WireId(){
			return (*FDC0_wireid);
		}
		vector<double > getFDC0TDC(){
			return (*FDC0_TDC);
