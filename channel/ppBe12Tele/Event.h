...
class GateCut{
	private:
		...
		void loadCut(){
                        ...
                        gROOT->ProcessLine(".x inputRootfiles/cutTeleProton.C");
		...
		void getCut(){
			...
			teleProtonCut	= (TCutG*)gROOT->GetListOfSpecials()->FindObject("teleProton");
	...
	public:
		...
		bool isTeleProton(double E,double dE){
			return teleProtonCut->IsInside(E,dE);
		}
...
class Event{
	private:
		...
		bool isProton(){
			...
			return gc->isTeleProton(rf->getTeleCsiE(),rf->getTeleDssdE());
		}
		bool isTargetArea(){
			return gc->isInsideTargetArea(rf->getTargetX(),rf->getTargetY());
		}
		bool isPRAngle(){
			return gc->isInsidePRAngle(rf->getResAngle(),rf->getTeleAngle());
		}
		bool isHodPid(){
			return gc->isHodPid(rf);
		}
		bool isFDC0(){
			return (rf->getResAngle()>0.5);
		}
	...
	public:
		...
		bool selectEvent(){
			...
			//return isBeam()&&isProton()&&isTargetArea()&&isPRAngle();
			return isBeam()&&isProton()&&isTargetArea()&&isFDC0();
			//return isBeam();
			//return isProton();
			//return isTargetArea();
			//return isPRAngle();
