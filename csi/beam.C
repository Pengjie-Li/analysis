...
int main(int argc, char *argv[])
{
	...
	SILICONSPara->LoadParameter((char*)"/media/Projects/RIKEN_Cluster_2018/lipj/exp_201805/anaroot/db/SILICONS.xml");
	...
	//TFile *dcin = new TFile(Form("/media/Projects/RIKEN_Cluster_2018/lipj/exp_201805/anaroot/users/dc/root/dcdist0%d_100000.root",runnum),"READ");
	if(dcin->IsOpen()){
		gROOT->cd();
		TH1* hist = NULL;
		// BDC1
	...
	TString outputfilename=Form("run0%d_Beamv8.root",runnum);
	...
	tree_BEAM->Branch("FDC0_nhits",&FDC0_nhits);
	tree_BEAM->Branch("FDC0_layer_hits",&FDC0_layer_hits);
	tree_BEAM->Branch("FDC0_nlayerx",&FDC0_nlayerx);
	tree_BEAM->Branch("FDC0_nlayery",&FDC0_nlayery);
	tree_BEAM->Branch("FDC0_TDC",&FDC0_TDC);
	tree_BEAM->Branch("FDC0_layerid",&FDC0_layerid);
	tree_BEAM->Branch("FDC0_wireid",&FDC0_wireid);
	...
	tree_BEAM->Branch("FDC0_Track_dl",&FDC0_Track_dl);
	tree_BEAM->Branch("FDC0_Track_XY",&FDC0_Track_XY);
	tree_BEAM->Branch("FDC0_Track_Z",&FDC0_Track_Z);
	tree_BEAM->Branch("FDC0_Track_dXY",&FDC0_Track_dXY);
	tree_BEAM->Branch("FDC0_X",&FDC0_X);
	tree_BEAM->Branch("FDC0_Y",&FDC0_Y);
	tree_BEAM->Branch("FDC0_ThetaX",&FDC0_ThetaX);
	tree_BEAM->Branch("FDC0_ThetaY",&FDC0_ThetaY);
	...
	tree_BEAM->Branch("SBV_X",&SBV_X);
	tree_BEAM->Branch("SBV_Y",&SBV_Y);
	tree_BEAM->Branch("SBV_ThetaX",&SBV_ThetaX);
	tree_BEAM->Branch("SBV_ThetaY",&SBV_ThetaY);

	// Telescope
	tree_BEAM->Branch("Silicons_ID",&Silicons_ID);
	tree_BEAM->Branch("Silicons_X",&Silicons_X);
	tree_BEAM->Branch("Silicons_Y",&Silicons_Y);
	tree_BEAM->Branch("Silicons_Z",&Silicons_Z);
	tree_BEAM->Branch("Silicons_Time",&Silicons_Time);
	tree_BEAM->Branch("Silicons_Energy",&Silicons_Energy);
	tree_BEAM->Branch("Silicons_Multiplicity",&Silicons_Multiplicity);

	tree_BEAM->Branch("SiLF_rawADC",&SiLF_rawADC);
	tree_BEAM->Branch("SiLB_rawADC",&SiLB_rawADC);
	tree_BEAM->Branch("SiRF_rawADC",&SiRF_rawADC);
	tree_BEAM->Branch("SiRB_rawADC",&SiRB_rawADC);

	tree_BEAM->Branch("SiLF_rawTDC",&SiLF_rawTDC);
	tree_BEAM->Branch("SiLB_rawTDC",&SiLB_rawTDC);
	tree_BEAM->Branch("SiRF_rawTDC",&SiRF_rawTDC);
	tree_BEAM->Branch("SiRB_rawTDC",&SiRB_rawTDC);

	tree_BEAM->Branch("CsILL_rawADC",&CsILL_rawADC);
	tree_BEAM->Branch("CsILRT_rawADC",&CsILRT_rawADC);
	tree_BEAM->Branch("CsILRB_rawADC",&CsILRB_rawADC);

	tree_BEAM->Branch("CsIRLT_rawADC",&CsIRLT_rawADC);
	tree_BEAM->Branch("CsIRLB_rawADC",&CsIRLB_rawADC);
	tree_BEAM->Branch("CsIRRT_rawADC",&CsIRRT_rawADC);
	tree_BEAM->Branch("CsIRRB_rawADC",&CsIRRB_rawADC);

	tree_BEAM->Branch("CsILL_rawTDC",&CsILL_rawTDC);
	tree_BEAM->Branch("CsILRT_rawTDC",&CsILRT_rawTDC);
	tree_BEAM->Branch("CsILRB_rawTDC",&CsILRB_rawTDC);

	tree_BEAM->Branch("CsIRLT_rawTDC",&CsIRLT_rawTDC);
	tree_BEAM->Branch("CsIRLB_rawTDC",&CsIRLB_rawTDC);
	tree_BEAM->Branch("CsIRRT_rawTDC",&CsIRRT_rawTDC);
	tree_BEAM->Branch("CsIRRB_rawTDC",&CsIRRB_rawTDC);
