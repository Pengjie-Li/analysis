...
bool Pixel::Bivariate_Least_Square_Fit(double EPS)
{
	Init();
	Precal();
...
{
	Remove(400);
	status=Least_Square_Fit();
	if(!status) cout<<"ERROR: Least square fitting failed after second fitting"<<endl;

	Remove(200);
	status=Least_Square_Fit();
	if(!status) cout<<"ERROR: Least square fitting failed after third fitting"<<endl;

	Remove(100,100,100);
	status=Least_Square_Fit();
	if(!status) cout<<"ERROR: Least square fitting failed after fourth fitting"<<endl;

	Remove(100,50,100);
	status=Least_Square_Fit();
	if(!status) cout<<"ERROR: Least square fitting failed after fifth fitting"<<endl;
...
bool Pixel::Remove(double Residue1,double Residue2,double Residue3)
{
	n=xarray.size();
	...
	for(int i=0;i<n;i++)
	{
		...
		double dy=y-k*x-b;
		double dy2=sqr(dy);
		if(x<500) cout<<dy<<endl;
		//if(dy2<Residue2||x<900||x>1800)
		if((x<900&&dy2<Residue1)||(x>899&&x<1700&&dy2<Residue2)||(x>1699&&dy2<Residue3))
		{
			tempx.push_back(x);
			tempy.push_back(y);
			tempxe.push_back(0);
