...
{
	TString outputname=Form("./runs/run%d_fit_calibration_parameters.txt",runnum);
...
void SCM::LoadFitPar()
{
	...
	ifstream in;
	TString inputname=Form("./runs/run%d_fit_calibration_parameters.txt",runnum);
...
void SCM::OverAllSCM()
{
	...
	TString outputname=Form("runs/run%d_scm_par.txt",runnum);
	ofstream fout(outputname, ios_base::out);

	//for(int side=0;side<2;side++)
	for(int side=0;side<1;side++)
		//for(int strip=0;strip<32;strip++)
		for(int strip=31;strip<32;strip++)
		{
			CalSCM(side,strip);
		...
		//if(pixel[strip][refback].n<100)
		{
			for(int i=0;i<32;i++)
				for(int j=0;j<32;j++)
				{
					...
					//if((pixel[i][refback].n<35)||(pixel[i][j].n<35)||(pixel[strip][j].n<35)) continue;
					//cout<<pixel[i][refback].status<<"  "<<pixel[i][j].status<<"  "<<pixel[strip][j].status<<endl;
