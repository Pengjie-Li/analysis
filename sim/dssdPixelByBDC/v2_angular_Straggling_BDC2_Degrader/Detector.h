...
class DegraderDetector:public Detector{
	private:
		const static double DegraderZ = 3920;
		//const static double DSSDZ = env->GetValue("DSSDZ",3850.);
	public:

		bool isAddAngularStraggling;
		double angDegraderSigma;

		DegraderDetector(BeamGenerator *beam){
			Z = DegraderZ;
			setDetectorPlane();
			calculateCrossPosition(beam);

			angDegraderSigma = env->GetValue("angDegraderSigma",0.);
			isAddAngularStraggling = env ->GetValue("isDegrederAddAng",false);
			if(isAddAngularStraggling) addAngularStraggling(beam);
		}

		void addAngularStraggling(BeamGenerator *beam){
			// This straggling wont change position in Degrader, but will change the direction of Beam
			double newDistance = 100; // mm  mean after 10 cm after BDC1 Plane
			double spaceDegraderSigma = angDegraderSigma*newDistance/1000; // 1000 means angle here is mrad

			Double_t deltaX=r.Gaus(0,spaceDegraderSigma);
			Double_t deltaY=r.Gaus(0,spaceDegraderSigma);
			TVector3 virtualDegrader;
			virtualDegrader.SetXYZ(virtualX,virtualY,Z);
			TVector3 beamVector = beam->getBeamVector();
			TVector3 newDegrader = virtualDegrader + newDistance*beamVector;
			double newX = newDegrader.X()+deltaX;
			double newY = newDegrader.Y()+deltaY;
			newDegrader.SetX(newX);
			newDegrader.SetY(newY);
			TVector3 newBeamVector = (newDegrader - virtualDegrader).Unit();


			//cout<<" show Angle Difference between Before and After:"<<endl;
			//cout<<newBeamVector.Angle(beamVector)/sqrt(2)<<endl;
			beam->setDirect(newBeamVector);
			beam->setPointXYZ(virtualDegrader);
