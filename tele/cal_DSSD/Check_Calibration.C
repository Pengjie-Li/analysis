...
//	double rbf=-1665.092353;
	...
	TString outputname=Form("run%d_checking_calibration_v2.root",runnum); // ped fitting parameter
	...
	for (Long64_t ientry=0; ientry<nentries;ientry++) {
		...
		for(int id=0;id<32;id++)
		{
			...
			double lfcal=lkf*lfscm+lbf;
			double lbcal=lkb*lbscm+lbb;
			...
			double rfcal=rkf*rfscm+rbf;
			double rbcal=rkb*rbscm+rbb;
...
void Load(int lr,double k[4][32],double b[4][32],double ek[4][32],double eb[4][32])
{
	ifstream in;
	TString par_name;
	if(lr==0) par_name="/media/Projects/RIKEN_Cluster_2018/lipj/exp_201805/anaroot/users/analysis/macros/scm/runs/DSSD_SCM_Left.txt";
	else  par_name="/media/Projects/RIKEN_Cluster_2018/lipj/exp_201805/anaroot/users/analysis/macros/scm/runs/DSSD_SCM_Right.txt";
	in.open(par_name);
	//      if(!in.open(par_name))
	//      {
