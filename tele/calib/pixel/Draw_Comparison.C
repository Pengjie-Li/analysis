...
void Draw_Comparison(int runnum, double CsIid[32][32],TString CsIname,TString peak);
int CheckEvent(double Target_X,double Target_Y,TString peak);
void Draw_Histo(int runnum,double CsIid[32][32],TString CsIname,TString peak);
void Load(double LLid[32][32]);
void Load(int runnum, double CsIid[32][32],TString CsIname,TString peak);
void Check(int fid,int bid,double Target_X,double Target_Y,double SiLF_E[32],double SiLB_E[32],int &spixel,int &ssum);
int Area(double CsI[32][32],int fid,int bid);
...
/**
 * Step 1:pixel, 2 pixel Correction , 3 general
 * As now, change all into general
 * 1. find text and determin the area
 * 2. open file and fill histogram
 */

void Draw_Comparison()
{
//	Draw_Comparison(runnum,RLTid,"RLT","peak1");
//	Draw_Comparison(runnum,RLBid,"RLB","peak1");
//	Draw_Comparison(runnum,RRTid,"RRT","peak1");
//	Draw_Comparison(runnum,RRBid,"RRB","peak1");

	Draw_Comparison(runnum,LLid,  "LL","peak1");
	Draw_Comparison(runnum,LRTid,"LRT","peak1");
	Draw_Comparison(runnum,LRBid,"LRB","peak1");

}
void Draw_Comparison(int runnum, double CsIid[32][32],TString CsIname,TString peak)
{
	Load(runnum,CsIid,CsIname,peak);
	Draw_Histo(runnum,CsIid,CsIname,peak);

}
void Load(int runnum, double CsIid[32][32],TString CsIname,TString peak){
	TString parname="result/Fix_"+peak+"_"+CsIname+Form("pixel_run0%d.txt",runnum);
	cout<<parname<<endl;
	ifstream in;
	in.open(parname);
...
void Draw_Histo(int runnum,double CsIid[32][32],TString CsIname,TString peak)
{
	...
	TH1F *hsum=new TH1F("hsum"+CsIname,"hsum"+CsIname,1400,-200,1200);
...
//	nentries=10000;
	...
	for (Long64_t ientry=0; ientry<nentries;ientry++) {
		...
		if(runnum==596||runnum==599){
			...
			fid=SiRF_id;
			bid=SiRB_id;
		}else cout<<"Error in Runnum"<<endl;


		if(Area(CsIid,fid,bid)){
			ssum=CheckEvent(Target_X,Target_Y,peak);
		}

		if(ssum==1){
			if(CsIname=="LL") hsum->Fill(CsI_LL_E);
			if(CsIname=="LRT") hsum->Fill(CsI_LRT_E);
			if(CsIname=="LRB") hsum->Fill(CsI_LRB_E);

			if(CsIname=="RLT") hsum->Fill(CsI_RLT_E);
			if(CsIname=="RLB") hsum->Fill(CsI_RLB_E);

			if(CsIname=="RRT") hsum->Fill(CsI_RRT_E);
			if(CsIname=="RRB") hsum->Fill(CsI_RRB_E);
		}
	...
	TCanvas *cpixelsum=new TCanvas("cpixelsum"+CsIname,"cpixelsum"+CsIname);
	cpixelsum->cd();
	hsum->Draw();
	cpixelsum->Write();
...
int CheckEvent(double Target_X,double Target_Y,TString peak){
	...
	if(peak=="peak5"){
		if(sqrt((Target_X+2.13)*(Target_X+2.13)+(Target_Y+1.6)*(Target_Y+1.6))<15) return 1;
		else return 0;

	}else if(peak=="peak2"){
		if(!(sqrt((Target_X+2.13)*(Target_X+2.13)+(Target_Y+1.6)*(Target_Y+1.6))<15)&&sqrt((Target_X+2.13)*(Target_X+2.13)+(Target_Y+1.6)*(Target_Y+1.6))<21&&!(((Target_X+2.13)<4&&(Target_X+2.13)>-4&&(Target_Y+1.6)>15&&(Target_Y+1.6)<25))) return 1;
		else return 0;

	}else if(peak=="peak1"){
		if((Target_X+2.13)<30&&(Target_X+2.13)>-30&&(Target_Y+1.1)<25&&(Target_Y+1.1)>-35&&!(sqrt((Target_X+2.13)*(Target_X+2.13)+(Target_Y+1.6)*(Target_Y+1.6))<20)&&!(((Target_X+2.13)<4&&(Target_X+2.13)>-4&&(Target_Y+1.6)>15&&(Target_Y+1.6)<25))) return 1;
		else return 0;
...
int Area(double CsIid[32][32],int fid,int bid)
{
	if(CsIid[fid][bid]>0) return 1;
	return 0;
}
void Check(int fid,int bid,double Target_X,double Target_Y,double SiLF_E[32],double SiLB_E[32],int &spixel,int &ssum)
{
