...
void drawPad(){
		...
		for (int i = 0; i < 2; ++i) {
			...
			int cid =i;
			//TString hName = Form("hDssd%d", cid);
			TString hName = Form("hApDeg%d", cid);
			...
			if(i==0){
				...
				hCheck[0]->GetYaxis()->SetRangeUser(1,1E5);
				hCheck[0]->GetXaxis()->SetRangeUser(0,20);
				hCheck[0]->Draw();
			}
		...
		drawVerticleLine(119.51);
		drawVerticleLine(152.16);

		drawText("119.51" , 0.4, 0.8);
		drawText("152.16" , 0.6, 0.8);
}
