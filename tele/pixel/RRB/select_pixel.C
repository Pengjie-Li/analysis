...
void select_pixel(){ 
	...
	TString outputname=Form("pixel%d_%d_run0%d_Beamv8.root",fid,bid,runnum);
	...
	double pixelRRT;
	double pixelRRB;
//	tree->Branch(Form("pixel%d_%dF",fid,bid),  &pixelF);
//	tree->Branch(Form("pixel%d_%dB",fid,bid),  &pixelB);
//	tree->Branch(Form("pixel%d_%dLL",fid,bid),  &pixelLL);
//	tree->Branch(Form("pixel%d_%dLRT",fid,bid),  &pixelLRT);
//	tree->Branch(Form("pixel%d_%dLRB",fid,bid),  &pixelLRB);
	...
	for (Long64_t ientry=0; ientry<nentries;ientry++) {
		...
		if((*SiRF_rawADC)[fid]>500&&(*SiRB_rawADC)[bid]>500)
		{
			pixelF=(*SiRF_rawADC)[fid];
			pixelB=(*SiRB_rawADC)[bid];
			pixelRLT=CsIRLT_rawADC;
			pixelRLB=CsIRLB_rawADC;
			pixelRRT=CsIRRT_rawADC;
			pixelRRB=CsIRRB_rawADC;
