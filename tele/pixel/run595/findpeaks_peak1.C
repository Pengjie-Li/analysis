...
void pixel(int fid,int bid,int isLR)
{

	//TString inputname=Form("pixelrootfiles/pixel%d_%d_run0%d_insidetarget.root",fid,bid,runnum);
	TString inputname[9];
	if(isLR==0){
	inputname[0]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid,bid,runnum);
	inputname[1]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid+1,bid,runnum);
	inputname[5]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid+2,bid,runnum);

	inputname[2]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid,bid+1,runnum);
	inputname[3]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid+1,bid+1,runnum);
	inputname[6]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid+2,bid+1,runnum);

	inputname[4]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid,bid+2,runnum);
	inputname[8]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid+1,bid+2,runnum);
	inputname[7]=Form("pixelrootfiles/Lpixel%d_%d_run0%d.root",fid+2,bid+2,runnum);

	}else{
	inputname[0]=Form("pixelrootfiles/Rpixel%d_%d_run0%d.root",fid,bid,runnum);
	inputname[1]=Form("pixelrootfiles/Rpixel%d_%d_run0%d.root",fid+1,bid,runnum);
	inputname[2]=Form("pixelrootfiles/Rpixel%d_%d_run0%d.root",fid,bid+1,runnum);
	inputname[3]=Form("pixelrootfiles/Rpixel%d_%d_run0%d.root",fid+1,bid+1,runnum);
	
	}
	//cout<<inputname<<endl;
	//TFile *f = new TFile(inputname,"READ");
	TChain *tree=new TChain("tree");
	for(int i=0;i<9;i++) tree->Add(inputname[i]);
...
void Draw(int fid,int bid,int runnum,TString CsI,TTree *tree)
{
	...
	//TString condition="(("+peak1+")||("+peak2+"))&&"+ADCLimit;
	//TString condition="";
	TString condition=ADCLimit;
