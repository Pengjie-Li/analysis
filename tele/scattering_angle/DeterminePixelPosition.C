...
class ReadFile{
	...
	public:
		...
		void setBranch(){
			...
			tree->SetBranchAddress("CsI_LL_E",&CsI_LL_E);
			tree->SetBranchAddress("CsI_LRT_E",&CsI_LRT_E);
			tree->SetBranchAddress("CsI_LRB_E",&CsI_LRB_E);
			tree->SetBranchAddress("CsI_RRT_E",&CsI_RRT_E);
			tree->SetBranchAddress("CsI_RRB_E",&CsI_RRB_E);
			tree->SetBranchAddress("CsI_RLT_E",&CsI_RLT_E);
			tree->SetBranchAddress("CsI_RLB_E",&CsI_RLB_E);
...
//		}
		...
		for(int i=366;i<456;i++){
			
			if(i==380) continue;
			if(i>386&&i<426) continue;
			if(i==431||i==432||i==439||i==440||i==451) continue;
			pixelPosition[i] = new PixelPosition(i,maxEventNumber,isLR,fid,bid);
		}
