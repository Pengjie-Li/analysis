...
class ReadFile{
	...
	public:
		...
		void setOutputBranch(TTree *treeOutput){

			treeOutput->Branch("Trig_DSB",&Trig_DSB);
			treeOutput->Branch("Trig_BxESPRIxTEL",&Trig_BxESPRIxTEL);
			treeOutput->Branch("Trig_BxESPRI",&Trig_BxESPRI);
			treeOutput->Branch("Trig_BxTEL",&Trig_BxTEL);
			treeOutput->Branch("Trig_BxHODP",&Trig_BxHODP);
			treeOutput->Branch("Trig_BxHODF",&Trig_BxHODF);
			treeOutput->Branch("Trig_BxNEBULA",&Trig_BxNEBULA);


			treeOutput->Branch("FDC0_X",&FDC0_X);
			treeOutput->Branch("FDC0_Y",&FDC0_Y);

			treeOutput->Branch("multiplicityLF",&multiplicityLF);
			treeOutput->Branch("multiplicityLB",&multiplicityLB);
			treeOutput->Branch("multiplicityRF",&multiplicityRF);
			treeOutput->Branch("multiplicityRB",&multiplicityRB);

			treeOutput->Branch("SiLF_CAL",&SiLF_CAL,"SiLF_CAL[40]/D");
			treeOutput->Branch("SiLB_CAL",&SiLB_CAL,"SiLB_CAL[40]/D");
			treeOutput->Branch("SiRF_CAL",&SiRF_CAL,"SiRF_CAL[40]/D");
			treeOutput->Branch("SiRB_CAL",&SiRB_CAL,"SiRB_CAL[40]/D");


			treeOutput->Branch("F13_Charge",&F13_Charge);
			treeOutput->Branch("TOF713",&TOF713);

			treeOutput->Branch("CsILL_CAL",&CsILL_CAL);
			treeOutput->Branch("CsILRT_CAL",&CsILRT_CAL);
			treeOutput->Branch("CsILRB_CAL",&CsILRB_CAL);
			treeOutput->Branch("CsIRRT_CAL",&CsIRRT_CAL);
			treeOutput->Branch("CsIRRB_CAL",&CsIRRB_CAL);
			treeOutput->Branch("CsIRLT_CAL",&CsIRLT_CAL);
			treeOutput->Branch("CsIRLB_CAL",&CsIRLB_CAL);
...
class PixelPosition{
	public :
		...
		bool isSiEnergyGood(){
			...
			if(fid!=-1) siEnergy = isLR?rf->SiRF_CAL[fid]:rf->SiLF_CAL[fid];
			if(bid!=-1) siEnergy = isLR?rf->SiRB_CAL[bid]:rf->SiLB_CAL[bid];
		...
		bool isCsIEnergyGood(){
			double CsIEnergy = -1;
			if(isLR){
				CsIEnergy = (rf->CsIRLT_CAL>rf->CsIRLB_CAL)?rf->CsIRLT_CAL:rf->CsIRLB_CAL;
			}else{
				CsIEnergy = (rf->CsILRT_CAL>rf->CsILRB_CAL)?rf->CsILRT_CAL:rf->CsILRB_CAL;
			}
...
int StripPositionRun595(){
	...
	//pixelPosition = new PixelPosition(595,maxEventNumber,isLR,fid,bid);
	
	PixelPosition *pixelPosition[100];
	int isLR = 0;
	int maxEventNumber = 5000000;
	//for(int id = 25;id<32;id++){
	//for(int id = 12;id<19;id++){
	for(int id = 12;id<13;id++){
		pixelPosition[id] = new PixelPosition(595,maxEventNumber,isLR,id,-1);
		//pixelPosition[id] = new PixelPosition(595,maxEventNumber,isLR,-1,id);
	}
